...
const participantId = sessionStorage.getItem('musiczoom-participant') ?? crypto.randomUUID();
...
function readPreferences(): Preferences {
  try {
    const raw = JSON.parse(localStorage.getItem('musiczoom-preferences') ?? '{}');
...
function savePreferences(): void { localStorage.setItem('musiczoom-preferences', JSON.stringify(prefs)); }
...
$('copy-invite')?.addEventListener('click', () => {
  ...
  const link = `musiczoom://join/${currentCode}`;
  void window.musiczoom.copyText(link).then(() => {
...
try {
  const savedDrawerWidth = parseInt(localStorage.getItem('musiczoom-session-workspace-width') || '460', 10);
...
function setSessionWorkspaceOpen(open: boolean): void {
  ...
  if (open) {
    ...
    const savedTab = localStorage.getItem('musiczoom-session-workspace-tab') || 'lyrics';
...
window.addEventListener('mouseup', () => {
  ...
  if (currentWidth) {
    try {
      localStorage.setItem('musiczoom-session-workspace-width', Math.round(currentWidth).toString());
    } catch {
      // ignore
    }
  }
...
// ========================================================
// PROJECT ACTIVITY & CHRONOLOGICAL HISTORY ENGINE
// ========================================================
...
function getActivityIconSvg(type: ProjectActivityType): string {
  switch (type) {
...
function renderProjectActivities(): void {
  if (!activeProject) return;
  const activities = Array.isArray(activeProject.activities) ? activeProject.activities : [];
  setText('overview-activity-count', activities.length.toString());

  const listEl = $('overview-activity-list');
  if (!listEl) return;
  listEl.innerHTML = '';
...
function openActivityDialog(): void {
  if (!activeProject) return;
  const dialog = $('project-activity-dialog');
  if (!dialog) return;

  const subtitle = $('activity-dialog-subtitle');
  if (subtitle) {
    subtitle.textContent = `Chronological creative history for "${activeProject.name}"`;
  }

  const searchInput = $<HTMLInputElement>('activity-search-input');
  if (searchInput) {
    searchInput.value = '';
  }
...
function renderActivityDialog(query = ''): void {
  if (!activeProject) return;
  const activities = Array.isArray(activeProject.activities) ? activeProject.activities : [];
  const listEl = $('activity-dialog-list');
  if (!listEl) return;
  listEl.innerHTML = '';
  ...
  setText('activity-dialog-count-badge', `${filtered.length} Event${filtered.length === 1 ? '' : 's'}`);
...
// Activity Dialog Event Listeners
$('btn-overview-view-activity')?.addEventListener('click', () => {
  openActivityDialog();
});

$('btn-close-activity-dialog')?.addEventListener('click', () => {
  $('project-activity-dialog')?.classList.add('hidden');
});

$('activity-search-input')?.addEventListener('input', (e) => {
  const q = (e.target as HTMLInputElement).value;
  renderActivityDialog(q);
});

// Close activity dialog on backdrop click
$('project-activity-dialog')?.addEventListener('click', (e) => {
  if (e.target === $('project-activity-dialog')) {
    $('project-activity-dialog')?.classList.add('hidden');
  }
});

// ========================================================
// IN-SESSION LIVE CHAT ENGINE
// ========================================================
...
function scrollChatToBottom(): void {
  const container = $('session-chat-messages');
  if (container) {
    container.scrollTop = container.scrollHeight;
  }
}

function updateChatUnreadBadge(): void {
...
function appendChatMessageToUi(msg: SessionChatMessage, isOutgoing: boolean): void {
  const container = $('session-chat-messages');
  if (!container) return;
...
function setSessionChatOpen(open: boolean): void {
  sessionChatOpen = open;
  $('session-chat-panel')?.classList.toggle('hidden', !open);
  $('toggle-session-chat')?.classList.toggle('active', open);
  $('call-view')?.classList.toggle('has-chat-open', open);
...
function resetChatUi(): void {
  ...
  unreadChatCount = 0;
  sessionChatMessages = [];
  $('session-chat-panel')?.classList.add('hidden');
  $('toggle-session-chat')?.classList.remove('active');
  $('call-view')?.classList.remove('has-chat-open');
  updateChatUnreadBadge();
...
$('toggle-session-chat')?.addEventListener('click', () => {
  setSessionChatOpen(!sessionChatOpen);
});

$('btn-close-session-chat')?.addEventListener('click', () => {
