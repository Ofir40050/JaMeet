...
describe('JaMeet Remote Bridge (Phase 1 Native Bridge)', () => {
  it('compiles and passes all native C bridge test suites', () => {
    ...
    expect(output).toContain('[PASS] test_seqlock_torn_write_detection');
    expect(output).toContain('[PASS] test_generation_epoch_resync');
    expect(output).toContain('[PASS] test_inactivity_and_heartbeat');
    expect(output).toContain('[PASS] test_multithreaded_concurrency');
    expect(output).toContain('[PASS] test_posix_shm_lifetime');
